use strict;
use warnings;
package Email::Sender::Mouse::Manual::QuickStart;
BEGIN {
  $Email::Sender::Mouse::Manual::QuickStart::VERSION = '0.110001';
}
# ABSTRACT: how to start using Email::Sender::Mouse right now
...
Email::Sender::Mouse::Manual::QuickStart - how to start using Email::Sender::Mouse right now
...
to Email::Sender::Mouse::Simple.  Email::Sender::Mouse::Simple tries to make a good guess
...
This is exactly the same as saying:
  ...
  Email::Sender::Mouse::Simple->send($email);

...but it's a lot easier to type.  You can use either one.

=head3 envelope information

We didn't have to tell Email::Sender::Mouse::Simple where to send the message.  If you
...
set those distinctly is important, and Email::Sender::Mouse::Simple lets you do this:
...
L<Email::Sender::Mouse::Success> object.  This object only rarely has much use.
...
be an object belonging to the class L<Email::Sender::Mouse::Failure>.  This object
will have a C<message> attribute describing the nature of the failure.  There
are several specialized forms of failure, like
L<Email::Sender::Mouse::Failure::Multi>, which is thrown when more than one error is
encountered when trying to send.  You don't need to know about these to use
Email::Sender::Mouse::Simple, though.  All you need to know is that C<sendmail>
...
For example:
  ...
  Email::Sender::Mouse::Simple->try_to_send($email, { ... });

  use Email::Sender::Mouse::Simple qw(try_to_sendmail);
  try_to_sendmail($email, { ... });

Some Email::Sender::Mouse transports can signal success if some, but not all,
recipients could be reached.  Email::Sender::Mouse::Simple does its best to ensure
that this never happens.  When you are using Email::Sender::Mouse::Simple, mail should
...
If Email::Sender::Mouse::Simple doesn't pick the transport you want, or if you have
...
via SMTP on an unusual port.  You can send mail like this:
  ...
  my $transport = Email::Sender::Mouse::Transport::SMTP->new({
...
Now, instead of guessing at what transport to use, Email::Sender::Mouse::Simple will
...
If you have a program that makes several calls to Email::Sender::Mouse::Simple, and
...
In that example, any message sent via Email::Sender::Mouse::Simple would be delivered
to a maildir in the current directory.

=head3 subclassing to change the default transport

If you want to use a library that will behave like Email::Sender::Mouse::Simple but
with a different default transport, you can subclass Email::Sender::Mouse::Simple and
replace the C<build_default_transport> method.

=head2 Testing

Email::Sender::Mouse::Simple makes it very, very easy to test code that sends email.
The simplest way is to do something like this:
  ...
  my @deliveries = Email::Sender::Mouse::Simple->default_transport->deliveries;

Now you've got an array containing every delivery performed through
Email::Sender::Mouse::Simple, in order.  Because you set the transport via the
environment, no other code will be able to force a different transport.

When testing code that forks, L<Email::Sender::Mouse::Transport::SQLite> can be used
...
Have a look at L<Email::Sender::Mouse::Manual>, where all the manual's documents are
listed.  You can also look at the documentation for L<Email::Sender::Mouse::Simple>
and the various Email::Sender::Mouse::Transport classes.
