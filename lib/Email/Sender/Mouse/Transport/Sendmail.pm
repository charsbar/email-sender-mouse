package Email::Sender::Mouse::Transport::Sendmail;
BEGIN {
  $Email::Sender::Mouse::Transport::Sendmail::VERSION = '0.110001';
}
use Mouse;
...
sub _sendmail_pipe {
  ...
  my $pipe;
  Email::Sender::Mouse::Failure->throw("couldn't open pipe to sendmail ($prog): $!")
...
sub send_email {
  ...
  print $pipe $email->as_string
    or Email::Sender::Mouse::Failure->throw("couldn't send message to sendmail: $!");

  close $pipe
    or Email::Sender::Mouse::Failure->throw("error when closing pipe to sendmail: $!");
...
Email::Sender::Mouse::Transport::Sendmail - send mail via sendmail(1)
...
To specify the location of sendmail:
  ...
  my $sender = Email::Sender::Mouse::Transport::Sendmail->new({ sendmail => $path });
