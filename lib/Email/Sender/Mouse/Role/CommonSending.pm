package Email::Sender::Mouse::Role::CommonSending;
BEGIN {
  $Email::Sender::Mouse::Role::CommonSending::VERSION = '0.110001';
}
use Mouse::Role;
# ABSTRACT: the common sending tasks most Email::Sender::Mouse classes will need

use Carp;
use Email::Abstract;
use Email::Sender::Mouse::Success;
...
sub send {
  ...
  try {
    ...
    if (try { $err->isa('Email::Sender::Mouse::Failure') } and ! $err->recipients) {
...
sub success {
  my $self = shift;
  my $success = Email::Sender::Mouse::Success->new(@_);
}
...
Email::Sender::Mouse::Role::CommonSending - the common sending tasks most Email::Sender::Mouse classes will need
...
Email::Sender::Mouse::Role::CommonSending provides a number of features that should
ease writing new classes that perform the L<Email::Sender::Mouse> role.  Instead of
...
A C<success> method will also be provided as a shortcut for calling:
  ...
  Email::Sender::Mouse::Success->new(...);
...
This method returns a new Email::Sender::Mouse::Success object.  Arguments passed to
