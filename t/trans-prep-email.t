...
my $prep_simple = Email::Sender::Mouse::Transport->prepare_email($simple);
is($prep_simple->as_string, $simple->as_string, 'simple - strings same');

# ABSTRACT
my $abstract      = Email::Abstract->new($email);
my $prep_abstract = Email::Sender::Mouse::Transport->prepare_email($abstract);
...
# STRING
my $prep_string = Email::Sender::Mouse::Transport->prepare_email($email);
is($prep_string->as_string, $email, 'string - strings same');

# STRING REF
my $copy = $email;
my $prep_string_ref = Email::Sender::Mouse::Transport->prepare_email(\$copy);
